...
export class SkyblockRevenantLevel extends ConditionHandler<SkyblockRevenantLevelOptions, number> {
  override getId(): string {
    return 'skyblock-zombie-slayer-level'
  }

  override getDisplayName(context: HandlerContext): string {
    return context.application.i18n.t(($) => $['discord.conditions.handler.zombie-level.title'])
  }

  override displayCondition(context: HandlerContext, options: SkyblockRevenantLevelOptions): string {
    return context.application.i18n.t(($) => $['discord.conditions.handler.zombie-level.formatted'], {
  ...
  override async meetsCondition(
    ...
    condition: SkyblockRevenantLevelOptions
    ...
    const highestExperience = profiles
      .map((profile) => profile.slayer?.slayer_bosses.zombie.xp ?? 0)
      // eslint-disable-next-line unicorn/no-array-reduce
      .reduce((a, b) => Math.max(a, b))
    const highestLevel = getSlayerLevel(highestExperience, 'zombie')
...
export type SkyblockRevenantLevelOptions = SkyblockProfileOptionType & { fromLevel: number; toLevel: number }
