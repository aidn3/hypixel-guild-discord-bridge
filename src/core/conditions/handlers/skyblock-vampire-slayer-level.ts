...
export class SkyblockVampireLevel extends ConditionHandler<SkyblockVampireLevelOptions, number> {
  override getId(): string {
    return 'skyblock-vampire-slayer-level'
  }

  override getDisplayName(context: HandlerContext): string {
    return context.application.i18n.t(($) => $['discord.conditions.handler.vampire-level.title'])
  }

  override displayCondition(context: HandlerContext, options: SkyblockVampireLevelOptions): string {
    return context.application.i18n.t(($) => $['discord.conditions.handler.vampire-level.formatted'], {
  ...
  override async meetsCondition(
    ...
    condition: SkyblockVampireLevelOptions
    ...
    const highestExperience = profiles
      .map((profile) => profile.slayer?.slayer_bosses.vampire.xp ?? 0)
      // eslint-disable-next-line unicorn/no-array-reduce
      .reduce((a, b) => Math.max(a, b))
    const highestLevel = getSlayerLevel(highestExperience, 'vampire')
...
export type SkyblockVampireLevelOptions = SkyblockProfileOptionType & { fromLevel: number; toLevel: number }
